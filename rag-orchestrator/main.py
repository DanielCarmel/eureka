...
app = FastAPI(title="RAG Orchestration API", description="Retrieval Augmented Generation System")
...
class QueryRequest(BaseModel):
    question: str
    sources: Optional[List[str]] = None  # Optional list of sources to query (jira, confluence, s3)
...
class MCPService:
    @staticmethod
    async def query_context(question: str, sources: Optional[List[str]] = None, max_results: int = 5):
...
class VectorDBService:
    @staticmethod
    async def query_similar_documents(question: str, collection_name: str, top_k: int = 5):
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                response = await client.post(f"{VECTOR_DB_URL}/query", json=request_data)
                ...
                if "documents" in response_data:
                    ...
                    logger.error(f"Unexpected Vector DB response format: {response_data}")
                    return []
        except Exception as e:
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if response.status_code == 200:
                ...
                services_status["llm_engine"] = {"status": "unhealthy", "reason": response.text}
                all_healthy = False
    except Exception as e:
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if response.status_code == 200:
                ...
                services_status["mcp_server"] = {"status": "unhealthy", "reason": response.text}
                all_healthy = False
    except Exception as e:
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if response.status_code == 200:
                ...
                services_status["vector_db"] = {"status": "unhealthy", "reason": response.text}
                all_healthy = False
    except Exception as e:
