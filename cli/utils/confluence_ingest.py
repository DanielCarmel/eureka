...
class ConfluenceIngestor:
    ...
    def __init__(self):
        """Initialize the Confluence ingestor with credentials from environment variables"""
    ...
    def fetch_space_pages(self, space_key: str, max_results: int = 100) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for page in pages:
                ...
                try:
                    ...
                    body_html = page_with_content.get('body', {}).get('storage', {}).get('value', '')
                    ...
                    processed_page = {
                        ...
                        "parent": parent_title,
                        "version": page_with_content.get('version', {}).get('number', 1)
                    }
