...
def chunk_document(
    text: str, metadata: Dict[str, Any], chunk_size: int = 512, chunk_overlap: int = 128
) -> List[Dict[str, Any]]:
    """
    ...
    for i, chunk_content in enumerate(
        chunks
    ):  # Renamed from chunk_text to chunk_content
        ...
        chunk = {
            "id": chunk_id,
            "text": chunk_content,  # Using chunk_content instead of chunk_text
            "metadata": {
                **metadata,
                "chunk_index": i,
