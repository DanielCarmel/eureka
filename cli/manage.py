...
logging.basicConfig(
    ...
    handlers=[
        RotatingFileHandler(
            "logs/cli.log", maxBytes=10485760, backupCount=5
        ),
...
jira_parser = subparsers.add_parser("ingest-jira", help="Ingest Jira issues into the vector database")
jira_parser.add_argument("--project", required=True, help="Jira project key")
jira_parser.add_argument("--max-issues", type=int, default=100, help="Maximum number of issues to ingest")
jira_parser.add_argument("--collection", default="documents", help="Vector DB collection name")
jira_parser.add_argument("--chunk-size", type=int, default=512, help="Size of text chunks")
jira_parser.add_argument("--chunk-overlap", type=int, default=128, help="Overlap between chunks")
jira_parser.add_argument("--force", action="store_true", help="Force re-ingestion even if exists")

# Add Confluence ingestion command
confluence_parser = subparsers.add_parser("ingest-confluence", help="Ingest Confluence spaces into the vector database")
confluence_parser.add_argument("--space", required=True, help="Confluence space key")
confluence_parser.add_argument("--max-pages", type=int, default=100, help="Maximum number of pages to ingest")
confluence_parser.add_argument("--collection", default="documents", help="Vector DB collection name")
confluence_parser.add_argument("--chunk-size", type=int, default=512, help="Size of text chunks")
confluence_parser.add_argument("--chunk-overlap", type=int, default=128, help="Overlap between chunks")
confluence_parser.add_argument("--force", action="store_true", help="Force re-ingestion even if exists")

# Add S3 ingestion command
s3_parser = subparsers.add_parser("ingest-s3", help="Ingest S3 documents into the vector database")
...
s3_parser.add_argument("--collection", default="documents", help="Vector DB collection name")
s3_parser.add_argument("--chunk-size", type=int, default=512, help="Size of text chunks")
s3_parser.add_argument("--chunk-overlap", type=int, default=128, help="Overlap between chunks")
s3_parser.add_argument("--max-files", type=int, default=100, help="Maximum number of files to ingest")
s3_parser.add_argument("--force", action="store_true", help="Force re-ingestion even if exists")

# Add rebuild index command
rebuild_parser = subparsers.add_parser("rebuild-index", help="Rebuild vector database collection")
rebuild_parser.add_argument("--collection", default="documents", help="Collection name to rebuild")

# Add test query command
query_parser = subparsers.add_parser("test-query", help="Test a query against the RAG system")
query_parser.add_argument("question", help="Question to ask")
query_parser.add_argument("--sources", nargs='+', help="Specific sources to query (jira, confluence, s3)")
query_parser.add_argument("--collection", default="documents", help="Vector DB collection to query")
query_parser.add_argument("--context-chunks", type=int, default=5, help="Number of context chunks to retrieve")
...
def handle_ingest_jira(args):
    ...
    try:
        ...
        logger.info(f"Successfully ingested {success_count} out of {len(issues)} Jira issues")
...
def handle_ingest_confluence(args):
    ...
    try:
        ...
        logger.info(f"Successfully ingested {success_count} out of {len(pages)} Confluence pages")
...
def handle_ingest_s3(args):
    ...
    logger.info(f"Starting S3 ingestion for bucket {args.bucket} with prefix {args.prefix}")
    ingestor = S3Ingestor()
    vector_db = VectorDBClient()
    ...
    try:
        ...
        logger.info(f"Successfully ingested {success_count} out of {len(files)} S3 files")
