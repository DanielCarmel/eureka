...
class JiraConnector(BaseConnector):
    ...
    def __init__(self, url: str, username: str, token: str):
        ...
        if url and username and token:
            try:
                self.client = Jira(url=url, username=username, password=token)
    ...
    def _extract_text_from_markup(self, content: str) -> str:
        ...
        except Exception as e:  # Changed from bare except to Exception
            logger.debug(f"Failed to parse markup with markdown: {e}")
        ...
        try:
            ...
            if "comment" in fields:
                for comment in fields["comment"].get("comments", []):
                    comment_text = self._extract_text_from_markup(
                        comment.get("body", "")
                    )
                    comments.append(
                        {
            ...
            document = {
                ...
                "content": self._extract_text_from_markup(
