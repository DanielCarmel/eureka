...
class S3Ingestor:
    ...
    def __init__(self):
        ...
        if all([self.aws_access_key_id, self.aws_secret_access_key]):
            try:
                ...
                endpoint_info = (
                    f" using endpoint {self.endpoint_url}" if self.endpoint_url else ""
                )
                logger.info(
                    f"Initialized S3 client for region {self.region_name}{endpoint_info}"
                )
    ...
    def fetch_s3_documents(
        self, bucket_name: str, prefix: str = "", max_files: int = 100
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            logger.info(
                f"Fetching objects from bucket {bucket_name} with prefix {prefix}"
            )
            ...
            for file_info in files:
                ...
                try:
                    ...
                    processed_file = {
                        ...
                        "key": key,
                        "size": file_info["size"],
                        "last_modified": file_info["last_modified"].strftime(
                            "%Y-%m-%d %H:%M:%S"
                        ),
                    }
